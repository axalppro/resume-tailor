...
/**
 * SectionCapabilitiesEditor — manages capability bullets with add/edit/delete.
 */
import { useState } from "react";
import type { Capability } from "@resume-tailor/shared-types";
...
interface DraftEntry extends Omit<Capability, "id"> {
  id: string;
}
  ...
  return (
    <div className="rounded-lg border border-brand/30 bg-brand/5 p-4">
      <label className="block text-xs">
        <span className="mb-1 block font-medium text-slate-700">Capability *</span>
        <input
          value={text}
          onChange={(e) => setText(e.target.value)}
          placeholder="React & TypeScript frontend development"
...
export function SectionCapabilitiesEditor({ items, onChange }: Props) {
    ...
    <div className="space-y-4">
      ...
      {items.length === 0 && !adding && !editing && (
        <p className="py-8 text-center text-sm text-slate-400">
          No capabilities yet. Click &quot;+ Add Capability&quot; to get started.
        </p>
      )}
      {items.map((entry) => (
