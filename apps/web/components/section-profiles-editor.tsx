...
/**
 * SectionProfilesEditor — manages profile text variants with add/edit/delete.
 */
import { useState } from "react";
import type { ProfileVariant } from "@resume-tailor/shared-types";
...
interface Props {
  items: ProfileVariant[];
  onChange: (items: ProfileVariant[]) => void;
}

interface DraftEntry extends Omit<ProfileVariant, "id"> {
  id: string;
}
  ...
  return (
    <div className="rounded-lg border border-brand/30 bg-brand/5 p-4">
      <label className="block text-xs">
        <span className="mb-1 block font-medium text-slate-700">Profile Text *</span>
        <textarea
          value={text}
          onChange={(e) => setText(e.target.value)}
          placeholder="Experienced software engineer with 10+ years of expertise in building scalable web applications..."
          rows={4}
          className="w-full rounded-md border border-slate-300 bg-white px-3 py-1.5 text-sm text-slate-800 shadow-inner leading-snug focus:border-brand focus:outline-none focus:ring-1 focus:ring-brand"
        />
      </label>
      <div className="mt-3 flex items-center gap-2 justify-end">
        ...
        >
          {initial ? "Update" : "Add Profile Text"}
        </button>
      </div>
    </div>
  );
  ...
  return (
    <div className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
      <div className="flex items-start justify-between gap-4">
        <p className="w-full max-w-lg text-sm text-slate-800 leading-snug">{entry.text}</p>
...
export function SectionProfilesEditor({ items, onChange }: Props) {
  ...
  const handleDelete = (id: string) => {
    if (confirm("Delete this profile text?")) {
    ...
    <div className="space-y-4">
      ...
      {items.length === 0 && !adding && !editing && (
        <p className="py-8 text-center text-sm text-slate-400">
          No profile texts yet. Click &quot;+ Add Profile Text&quot; to get started.
        </p>
      )}
      {items.map((entry) => (
