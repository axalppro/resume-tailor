...
/**
 * SectionAdditionalEditor — manages additional experience entries with add/edit/delete.
 * Uses the same shape as ExperienceSchema.
 */
import { useState } from "react";
import type { Experience as AdditionalEntry } from "@resume-tailor/shared-types";
...
interface Props {
  items: AdditionalEntry[];
  onChange: (items: AdditionalEntry[]) => void;
}

interface DraftEntry extends Omit<AdditionalEntry, "id"> {
  id: string;
}
    ...
    <div className="rounded-lg border border-brand/30 bg-brand/5 p-4">
      <div className="grid gap-3 sm:grid-cols-2">
        <label className="block text-xs">
          <span className="mb-1 block font-medium text-slate-700">Activity/Volunteer *</span>
          <input
            value={title}
        ...
        <label className="block text-xs">
          <span className="mb-1 block font-medium text-slate-700">Organization *</span>
          <input
            value={org}
...
export function SectionAdditionalEditor({ items, onChange }: Props) {
  ...
  const handleDelete = (id: string) => {
    if (confirm("Delete this additional experience?")) {
    ...
    <div className="space-y-4">
      ...
      {items.length === 0 && !adding && !editing && (
        <p className="py-8 text-center text-sm text-slate-400">
          No additional activities yet. Click &quot;+ Add Activity&quot; to get started.
        </p>
      )}
      {items.map((entry) => (
